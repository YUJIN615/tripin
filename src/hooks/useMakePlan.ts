...
// localStorage에 결과 저장
const savePlanResultToLocalStorage = (result: PlanCreateResponseType) => {
...
export const loadTripResultFromLocalStorage = (): PlanCreateResponseType | null => {
  if (typeof window !== "undefined") {
    try {
...
export const useMakePlan = () => {
  return useMutation<PlanCreateResponseType, Error, MakeTripParams>({
    mutationFn: async (params) => {
      const response = await apiClient.post(API_ENDPOINTS.plans, {
